...
class ReadyToRange(object):
    ...
    def loop(self):
        ...
        if status == POZYX_SUCCESS:
            ...
            if self.ledControl(device_range.distance) == POZYX_FAILURE:
                print("\033[93m ERROR: setting (remote) leds \033[0m")
        else:
    ...
    def ledControl(self, distance):
        ...
        for id in ids:
            status &= self.pozyx.setLed(1, (distance > range_step_mm), id)
            status &= self.pozyx.setLed(2, (distance > 2 * range_step_mm), id)
            status &= self.pozyx.setLed(3, (distance > 3 * range_step_mm), id)
            status &= self.pozyx.setLed(4, (distance > 4 * range_step_mm), id)
        return status
